...
public struct CGSpline: Equatable {
  ...
  init(kind: Kind, points: [CGPoint]) {
    ...
    if points.count < 2 {
      collinearity = Collinearity(kind: .point)
    } else if horizontal, vertical {
      collinearity = Collinearity(kind: .point)
    } else if horizontal {
      collinearity = Collinearity(kind: .horizontal)
    } else if vertical {
      collinearity = Collinearity(kind: .vertical)
    } else {
      if points[0].x != points[points.count - 1].x {
        ...
        for point in points {
          if !point.y.isAlmostEqual(to: CGFloat(m * Double(point.x) + b), tolerance: tolerance) {
            collinearity = Collinearity(kind: .curve)
            return
          }
        }
        collinearity = Collinearity(kind: .slope, m: m, b: b)
      } else {
        collinearity = Collinearity(kind: .curve)
      }
    }
  ...
  func clip(around line: CGLine, minimum: CGFloat, maximum: CGFloat) -> (CGSpline, CGFloat, CGFloat)? {
    ...
    if minX > 0 {
      if maxX < 1 {
        guard let result = split(at: [minX, maxX])[safe: 1] else { return nil }
        return (result, minX, maxX)
      } else {
        guard let (_, result) = split(at: minX) else { return nil }
        return (result, minX, 1)
      }
    } else {
      if maxX < 1 {
        guard let (result, _) = split(at: maxX) else { return nil }
      ...
      if let intersectionPoint = intersectionPoint {
        ...
        if group[0].collinearity.kind == .horizontal || group[0].collinearity.kind == .vertical && group[1].collinearity.kind == .curve {
          ...
          for i in 0 ..< validRoots.count {
            guard let (a, c) = group[1].split(at: CGFloat(validRoots[i])) else { return [] }
            ...
            if i < validRoots.count - 1 {
              guard let b = group[1].split(at: [CGFloat(validRoots[i]), CGFloat(validRoots[i + 1])])[safe: 1] else { return [] }
              result.append(b)
            }
          }
...
public extension CGPath {
  var splines: [CGSpline] {
    ...
    applyWithBlock {
      let elem = $0.pointee
      var elemPointsCount: Int?
      var splineKind: CGSpline.Kind?
      switch elem.type {
      case .moveToPoint:
